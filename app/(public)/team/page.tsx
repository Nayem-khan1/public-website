...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: buildMetadataAlternates("/team", locale),
    openGraph: {
      title: t("team.metaTitle"),
      description: t("team.metaDescription"),
      locale: getLocalizedMetadataLocale(locale),
      url: getLocalizedAbsoluteUrl("/team", locale),
    },
  };
}
...
export default async function TeamPage() {
  ...
  const teamMembers = await getTeamMembers(undefined, locale);
  const fallbackRole = t("common.instructor");

  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      <PageHeader
        title={t("team.headerTitle")}
        subtitle={t("team.headerSubtitle")}
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 border border-slate-100 group text-center p-8">
      ...
      <p className={`${roleColor[variant]} font-medium text-sm mb-3`}>
        {member.role || member.category || fallbackRole}
      </p>
