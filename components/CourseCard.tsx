...
import { formatCurrency, formatNumber, localizeCourseMode, localizeLevel, normalizeLocale } from "@/lib/i18n";
...
export function CourseCard({ course }: CourseProps) {
  ...
  const lessonsLabel = t("courses.lessons", { count: formatNumber(course.totalLessons, locale) });
    ...
    <div className="group bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 flex flex-col h-full hover:-translate-y-1">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-4 right-4">
          ...
          >
            {localizeCourseMode(course.mode, locale)}
          </Badge>
        </div>
      </div>
          ...
          <div className="flex flex-col">
            <span className="text-xs text-slate-500 line-through">
              {course.originalPrice
                ? formatCurrency(course.originalPrice, locale)
                : ""}
            </span>
