      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4 border-b border-slate-100 pb-6">
        <div>
          ...
          <div className="flex items-center gap-6 text-sm text-slate-500 font-medium">
            <span className="flex items-center gap-2 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-full"><ListIcon className="w-4 h-4" /> {sections.length} {t("courseDetails.modules")}</span>
            <span className="flex items-center gap-2 bg-rose-50 text-rose-700 px-3 py-1.5 rounded-full"><PlayCircle className="w-4 h-4" /> {totalLessons} {t("common.lessons")}</span>
          </div>
        </div>
      </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between w-full text-left gap-2 md:gap-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="pt-1 md:pt-0">
                      <h3 className="font-bold text-slate-900 text-lg group-hover:text-primary transition-colors">{section.title || t("courseDetails.curriculum")}</h3>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-xs text-slate-600 font-semibold mr-4 mt-2 md:mt-0">
                  <span className="bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm hidden md:inline-block transition-colors group-hover:border-primary/30">
                  ...
                  {lessons.map((lesson, lessonIndex) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <span className="text-[15px] font-medium text-slate-700 group-hover/item:text-primary transition-colors">{lesson.title}</span>
                          {lesson.is_preview && (
